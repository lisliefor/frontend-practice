<template>
  <div class="layout-wrapper">
    ...
    <div class="micor-apps-viewport">
      <div id="sub-menu-viewport">
        <div id="nav">
          <div v-for="art in arts" class="nav-li" :key="art.title" @click="linkClick(art)">{{art.title}}</div>
        </div>
      </div>
      <div id="sub-content-viewport"></div>
    </div>
  </div>
</template>
...
export default {
  ...
  data () {
    return {
      ...
      arts: [
        {
          title: '东京奥运金牌，含金量被曝光了！竟是垃圾回收产品，几乎不含金',
          url: '/sub-vue/1',
          id: 1
        },
        {
        ...
        {
          title: '为高通、英伟达等生产芯片，三星电子考虑上调晶圆代工价格',
          url: '/sub-react/1',
          id: 3
        }
      ]
  ...
  methods: {
    ...
    linkClick (e) {
      history.pushState(e, 'sub-react', e.url)
    },
    listenRouterChange () {
